				...
				<?php if ( is_active_sidebar( 'copyright-1' ) ) : ?>
					<div class="row">
						...
						<div class="col-12 col-md-6">
							...
							<?php printf( esc_html__( 'Built with', 'developry-lite' ) ); ?> <a href="<?php print esc_url( DOCUMENTATION_URL ); ?>"><?php printf( esc_html__( 'Developry', 'developry-lite' ) ); ?></a> theme.
							</p>
						</div>
					</div>
				<?php else : ?>
					<p class="text-center">
						&copy; <?php print esc_html( date( 'Y' ) ); ?> <a href="<?php print esc_url( home_url( '/' ) ); ?>"><?php bloginfo( 'name' ); ?></a>. <?php printf( esc_html__( 'All rights reserved.', 'developry-lite' ) ); ?>
						<br />
					</p>
				<?php endif; ?>
			</div>
