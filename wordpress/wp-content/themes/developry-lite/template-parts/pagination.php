...
<?php if ( is_single() ) : ?>
	...
	<nav class="pagination">
		<?php if ( is_a( $prev, 'WP_Post' ) ) : ?>
			<a href="<?php print esc_url( get_permalink( $prev->ID ) ); ?>" class="previous-link">
				...
				if ( ! empty( $title ) ) {
					print esc_html( get_the_title( $prev->ID ) );
				} else {
		...
		<?php if ( is_a( $next, 'WP_Post' ) ) : ?>
			<a href="<?php print esc_url( get_permalink( $next->ID ) ); ?>" class="next-link">
				<?php

				$title = get_the_title( $next->ID );

				if ( ! empty( $title ) ) {
					print esc_html( get_the_title( $next->ID ) );
				} else {
